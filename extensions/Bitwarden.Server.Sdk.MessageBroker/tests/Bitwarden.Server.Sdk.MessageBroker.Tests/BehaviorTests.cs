...
public abstract class BehaviorTests : IAsyncLifetime
{
    ...
    /// <summary>
    /// Asserts that both competing consumers received at least one message, verifying fair
    /// distribution. Backends where the scheduler does not guarantee interleaving (e.g. the
    /// in-memory channel) may override this to skip the assertion.
    /// </summary>
    protected virtual void AssertFairDistribution(ConcurrentBag<int> received1, ConcurrentBag<int> received2)
    {
    ...
    public async Task CompetingConsumersBothHandleMessages()
    {
        ...
        AssertFairDistribution(received1, received2);
    }
