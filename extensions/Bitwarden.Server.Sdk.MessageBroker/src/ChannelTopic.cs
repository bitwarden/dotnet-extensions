...
internal sealed class ChannelTopic<T> : IHostedService
{
    ...
    public ChannelTopic(IEnumerable<string> subscriptionNames, IEnumerable<ChannelEscrowRegistration<T>> escrowRegistrations, MessageBrokerMetrics metrics, string topicName)
    {
    ...
    /// <inheritdoc cref="ChannelEscrowRegistration{T}.RegisterWith"/>
    public void SetEscrowFallback(string subscriptionName, Func<Envelope<T>, CancellationToken, Task> fallback)
        => _subscribers.GetOrAdd(subscriptionName, _ => new Subscription()).EscrowFallback = fallback;

    /// <inheritdoc cref="ChannelEscrowRegistration{T}.RegisterWith"/>
    public void SetStartupRecovery(string subscriptionName, Func<ChannelWriter<Envelope<T>>, CancellationToken, Task> recovery)
    ...
    // to escrow before sealing writers so subscribers exit cleanly.
    ...
    public async Task StopAsync(CancellationToken cancellationToken)
    {
        foreach (var (_, sub) in _subscribers)
        {
            ...
            {
                var remaining = new List<Envelope<T>>();
    ...
    {
        public Channel<Envelope<T>> Channel { get; } = System.Threading.Channels.Channel.CreateUnbounded<Envelope<T>>();
        public Func<Envelope<T>, CancellationToken, Task>? EscrowFallback;
        public Func<ChannelWriter<Envelope<T>>, CancellationToken, Task>? StartupRecovery;
        public Func<IReadOnlyList<Envelope<T>>, CancellationToken, Task>? ShutdownDrain;
    }
}
