...
///  Important: The structure of this type must match the structure
///   of the Response type in the C# BitwardenLibrary, both in field type and order.
#[repr(C)]
pub struct Response {
    pub error: usize,
    ...
    // Ideally we'd have a separate type per return type?
    pub data1: Buffer,
    pub data2: Buffer,
    pub data3: Buffer,
}
...
impl Response {
    ...
    pub fn error(error: Error) -> Self {
        ...
        let (error, message) = match error {
            Error::InvalidInput(name) => (1, name.to_string()),
            Error::Protocol(e) => (2, format!("{:?}", e)),
        };
